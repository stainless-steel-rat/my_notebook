...
#### **基于pytorch的伪量化训练**
  ...
  >- **补充说明一下：伪量化其实在数值的范围上实现了量化的作用，但是在训练的计算过程中还是以float这个类型进行计算，训练的过程并不会降低计算量，所以叫做伪量化。**
  >- **为了实现STE算法，在pytorch中需要利用Function函数，自己设计这个op的forward和backward实现直接传递梯度的功能**
  >- **参考文档:https://zhuanlan.zhihu.com/p/158776813**
